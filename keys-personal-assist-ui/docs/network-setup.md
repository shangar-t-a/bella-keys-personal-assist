...
* `/api/bella-chat` targets `http://bella-chat-service:5000`
* `/api/ems` targets `http://expense-manager-service:8000`
...
All services run on the `bella-keys-network` bridge network. Containerized services communicate using Docker DNS resolution (e.g., `bella-chat-service:5000`, `expense-manager-service:8000`).
